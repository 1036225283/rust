...
// 512 bytes
__constant unsigned long maskLong[8] = {0,         fBytes(1), fBytes(2),
                                        fBytes(3), fBytes(4), fBytes(5),
                                        fBytes(6), fBytes(7)};
...
unsigned long SWAP512(const unsigned long val) {
  unsigned long tmp = (rotr64(val & 0x0000FFFF0000FFFFUL, 16UL) |
                       rotl64(val & 0xFFFF0000FFFF0000UL, 16UL));
  return (rotr64(tmp & 0xFF00FF00FF00FF00UL, 8UL) |
          rotl64(tmp & 0x00FF00FF00FF00FFUL, 8UL));
}
...
static int md_pad_128(unsigned long *msg, const long msgLen_bytes) {
  ...
  const unsigned int overhang =
      (((unsigned int)msgLen_bytes) - padLongIndex * 8); // 0
  msg[padLongIndex] &=
      maskLong[overhang]; // msg[24] = msg[24] & 0 -> 0's out this byte
  msg[padLongIndex] |= padLong[overhang]; // msg[24] = msg[24] | 0x1UL << 7 ->
                                          // sets it to 0x1UL << 7
...
static void sha512(unsigned long *input, const unsigned int length,
                   ulong *hash) {
...
static void sha256(__private const unsigned int *pass, int *pass_len,
